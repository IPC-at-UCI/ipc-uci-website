      ...
      <Section className="h-[1000px] bg-ipc-black flex justify-center items-center">
        <div>
          ...
          <div className="flex gap-[40px]">
            <EventThumbnail eventData={EVENT_DATA[0]} />
            <EventThumbnail eventData={EVENT_DATA[1]} />
            <EventThumbnail eventData={EVENT_DATA[0]} />
          </div>
        </div>
      </Section>
            ...
            {EVENT_DATA.map((event: EventObject, key: number) => {
              ...
              return (
                <div
                  ...
                  key={key}
                >
                  <div className={`overflow-y-hidden ${expanded ? "h-fit" : "h-[150px]"}`}>
                    <div>
                    ...
                    <div className="flex gap-4 overflow-x-scroll">
                      {event.images?.map((image: StaticImport, key: number) => (
                        <Image
                          ...
                          alt="Event image"
                          key={key}
                          className="h-[300px] w-fit rounded-2xl"
                        />
                      ))}
                    </div>
                  </div>
