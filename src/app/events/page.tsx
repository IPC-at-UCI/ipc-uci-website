...
export default function Page() {
  const [upcomingVisible, setUpcomingVisible] = useState<boolean>(true);
  const [pastVisible, setPastVisible] = useState<boolean>(false);

  const toggleUpcoming = () => {
    setUpcomingVisible(!upcomingVisible);
  };
  const togglePast = () => {
    setPastVisible(!pastVisible);
  };
    ...
    <div className="overflow-x-hidden">
      ...
      <Section className="min-h-[1000px] bg-ipc-blue flex justify-center">
        <div className="container mx-auto mb-[100px]">
          <div className="flex justify-between items-end mb-[40px] gap-[80px]">
            <h2 className="text-5xl font-bold text-ipc-white mt-[100px]">
              See all IPC events
            </h2>
            <div className="flex gap-3">
              <button
                onClick={toggleUpcoming}
                className={`text-ipc-white text-xl px-4 py-3 hover:bg-ipc-black duration-200 rounded-lg ${upcomingVisible && "bg-ipc-black"}`}
              >
                Upcoming Events
              </button>
              <button
                onClick={togglePast}
                className={`text-ipc-white text-xl px-4 py-3 hover:bg-ipc-black duration-200 rounded-lg ${pastVisible && "bg-ipc-black"}`}
              >
                Past Events
              </button>
            </div>
          ...
          <div className="flex flex-col gap-4 text-xl">
            {EVENT_DATA.map((event: EventObject, key: number) => {
              ...
              if ((upcomingVisible && !eventPassed(event.date)) || (pastVisible && eventPassed(event.date))) {
                return (
                  <div
                    id={event.id}
                  ...
                  >
                    <div className="overflow-y-hidden h-fit">
                      ...
                      {expanded && (
                        <div className="flex gap-4 overflow-x-scroll mt-7">
                          {event.images?.map(
                            (image: StaticImport, key: number) => (
                              <Image
                                src={image}
                                alt="Event image"
                                key={key}
                    ...
                    <div className="flex justify-end">
                      <button
                        className="mt-5 text-blue-500 font-medium hover:text-blue-700 hover:underline duration-200"
                        onClick={() => {
